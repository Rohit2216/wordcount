...
try {
const response = await fetch('http://localhost:8800/api/wordcount');
...
form.addEventListener('submit', async function (event) {
    ...
    try {
        await fetch('http://localhost:8800/api/wordcount', {
            method: 'POST',
            headers: {
...
async function editWordCount(id, url) {
    ...
    updateButton.onclick = async function () {
        ...
        try {
            await fetch(`http://localhost:8800/api/wordcount/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
